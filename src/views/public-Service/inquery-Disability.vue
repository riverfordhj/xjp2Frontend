<template>
    <div class="app-container">
        <div class="filter-container">
            <el-select v-model="typevalue" placeholder="残疾人群类型" clearable style="width: 150px" class="filter-item" @change="filterType" @clear = "getpeopleData">
                <el-option v-for="item in this.specialType" :key="item.type" :label="item.type" :value="item.type" />
            </el-select>
            <el-select v-model="levelvalue" placeholder="等级" clearable style="width: 150px" class="filter-item" @change="filterlevel" @clear = "getpeopleData">
                <el-option v-for="item in this.disabilitylevel" :key="item.class" :label="item.class" :value="item.class" />
            </el-select>
            <el-button class="filter-item" type="primary" icon="el-icon-search" @click="getpeopleData">残疾人群</el-button> 
			  <!-- 导出数据为xlsx格式表格 -->
...
<script>
import {getRoomId,getDisabilityType,getDisabilitylevel,getDisability } from '@/api/person.js';
...
export default {
    ...
    methods:{
     getTypeData() {
            getDisabilityType().then(response => {
    ...
    getlevelData() {
            getDisabilitylevel().then(response => {
                this.disabilitylevel = response
            }).catch(error => {
                console.log(error)
            })
     },
    getpeopleData() {  
        getDisability().then(response => {
    ...
    filterType(){
         if(this.typevalue !== null && this.levelvalue.length > 0) {
              this.filterdPersonHouseInfo = this.tempfilterData.filter( item => {
            return item.type === this.typevalue && item.level === this.levelvalue;
            })
         }else{
            this.filterdPersonHouseInfo = this.tempfilterData.filter( item => {
    ...
    filterlevel(){
         if(this.typevalue !== null && this.levelvalue.length > 0) {
            ...
            this.filterdPersonHouseInfo = this.tempfilterData.filter( item => {
            return item.level === this.levelvalue;
            })
        }
     } 
  }
